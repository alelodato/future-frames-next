...
export default function AdminPortfolio() {
  ...
  return (
    <div className="space-y-6 w-full max-w-5xl mx-auto px-4 sm:px-0">

      {/* Header */}
      <div className="flex items-center justify-between gap-3">
        <div>
          <h1 className="font-antonio text-2xl sm:text-3xl text-white">Portfolio</h1>
        ...
        <Link href="/admin/portfolio/nuovo"
          className="inline-flex items-center gap-2 rounded-full bg-violet-400 px-4 sm:px-5 py-2.5 font-montserrat text-xs font-semibold uppercase tracking-wide text-[#050211] shadow-[0_0_20px_rgba(167,139,250,0.5)] transition hover:bg-violet-300 flex-shrink-0">
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {projects.map((project) => (
            <div key={project.id}
              className="rounded-2xl border border-violet-500/15 bg-gradient-to-br from-violet-900/10 via-[#0d0b2a] to-slate-950/80 px-4 sm:px-5 py-4 shadow-[0_4px_20px_rgba(0,0,0,0.4)]">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {project.featured && (
                      <span className="rounded-full border border-amber-400/30 bg-amber-900/20 px-2 py-0.5 font-montserrat text-[0.55rem] uppercase tracking-[0.15em] text-amber-300 flex-shrink-0">
                        Featured
                      </span>
                    )}
                  </div>
                ...
                <div className="hidden sm:flex items-center gap-2 flex-shrink-0">
                  <button onClick={() => toggleFeatured(project.id, project.featured)}
                    className={`rounded-full px-3 py-1.5 font-montserrat text-[0.6rem] uppercase tracking-[0.15em] transition border ${project.featured
                  ...
                  <button onClick={() => togglePublished(project.id, project.published)}
                    className={`rounded-full px-3 py-1.5 font-montserrat text-[0.6rem] uppercase tracking-[0.15em] transition border ${project.published
              ...
              {/* Azioni mobile */}
              <div className="flex sm:hidden items-center gap-2 mt-3 pt-3 border-t border-white/5">
                <button onClick={() => toggleFeatured(project.id, project.featured)}
                  className={`flex h-9 w-9 items-center justify-center rounded-full border transition flex-shrink-0 ${project.featured
                      ? "border-amber-400/40 text-amber-400 bg-amber-900/20"
                ...
                <button onClick={() => togglePublished(project.id, project.published)}
                  className={`flex-1 rounded-full py-2 font-montserrat text-[0.6rem] uppercase tracking-[0.15em] transition border text-center ${project.published
                      ? "border-zinc-600 text-zinc-400 hover:border-red-500/50 hover:text-red-400"
                      : "border-emerald-500/40 text-emerald-400 hover:bg-emerald-900/20"
                    }`}>
                  {project.published ? "Nascondi" : "Pubblica"}
                ...
                <button onClick={() => deleteProject(project.id)}
                  className="flex h-9 w-9 items-center justify-center rounded-full border border-red-500/20 bg-red-900/10 transition hover:bg-red-900/30">
                  <i className="fa-solid fa-trash text-red-400 text-xs" />
                </button>
              </div>

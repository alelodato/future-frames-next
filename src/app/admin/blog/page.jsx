  ...
  return (
    <div className="space-y-6 w-full max-w-5xl mx-auto px-4 sm:px-0">
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          <h1 className="font-antonio text-2xl sm:text-3xl text-white">Blog</h1>
          ...
          {articles.map((article) => (
            <div key={article.id}
              ...
              <div className="flex items-start gap-3">
                {/* Status dot */}
                <div className={`mt-1.5 h-2 w-2 flex-shrink-0 rounded-full ${article.published ? "bg-emerald-400" : "bg-zinc-600"}`} />
                ...
                <div className="hidden sm:flex items-center gap-2 flex-shrink-0">
                  <button onClick={() => togglePublished(article.id, article.published)}
                    className={`rounded-full px-3 py-1.5 font-montserrat text-[0.6rem] uppercase tracking-[0.15em] transition border ${article.published
              ...
              <div className="flex sm:hidden items-center gap-2 mt-3 pt-3 border-t border-white/5">
                <button onClick={() => togglePublished(article.id, article.published)}
                  className={`flex-1 rounded-full px-3 py-2 font-montserrat text-[0.6rem] uppercase tracking-[0.15em] transition border text-center ${article.published
                      ? "border-zinc-600 text-zinc-400 hover:border-red-500/50 hover:text-red-400"
                      : "border-emerald-500/40 text-emerald-400 hover:bg-emerald-900/20"
                    }`}>
                  {article.published ? "Nascondi" : "Pubblica"}
